...
PRODUCTS = {
    ...
    "imz": {
        "name": "📱 IMZ",
        "items": {
            "imz_dinhvi":     {"name": "ĐỊNH VỊ",          "price": 60000},
            "imz_aimco_dv":   {"name": "AIM CỔ + DV",      "price": 100000},
            "imz_aimbody_dv": {"name": "AIMBODY + DV",     "price": 120000},
            "imz_aimco":      {"name": "AIM CỔ",           "price": 65000},
            "imz_aimbody":    {"name": "AIMBODY",          "price": 75000}
        }
    },
    "filza": {
        "name": "📂 FILZA",
        "items": {
            "filza_aimco_dv":      {"name": "AIM CỔ + DV",        "price": 120000},
            "filza_aimmagic_dv":   {"name": "AIMMAGIC + DV",      "price": 130000},
            "filza_aimbody_dv":    {"name": "AIMBODY + DV",       "price": 120000},
            "filza_aimco_aimbody": {"name": "AIM CỔ + AIMBODY",   "price": 150000},
            "filza_dinhvi":        {"name": "ĐỊNH VỊ",            "price": 50000}
        }
    },
    "adr": {
        "name": "🤖 ADR",
        "items": {
            "adr_aimco":   {"name": "AIM CỔ",   "price": 60000},
            "adr_dinhvi":  {"name": "ĐỊNH VỊ",  "price": 50000},
            "adr_aimbody": {"name": "AIMBODY",  "price": 80000},
            "adr_aimlock": {"name": "AIMLOCK",  "price": 100000},
            "adr_nhetam":  {"name": "NHẸ TÂM",  "price": 25000}
        }
    }
}
...
def get_products_keyboard(category_id):
    ...
    if category_id in ("imz", "filza", "adr"):
        markup.row(types.InlineKeyboardButton("🔙 Quay Lại", callback_data="file_menu_back"))
    else:
...
# Keyboard menu FILE (IMZ / FILZA) - dẫn vào luồng mua hàng chung (cat_)
def get_file_menu_keyboard():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton("📱 IMZ", callback_data="cat_imz"),
        types.InlineKeyboardButton("📂 FILZA", callback_data="cat_filza")
    )
    markup.row(types.InlineKeyboardButton("🤖 ADR", callback_data="cat_adr"))
    markup.row(types.InlineKeyboardButton("🔙 Quay Lại", callback_data="user_close_deposit_menu"))
    return markup
...
def show_file_menu(message):
    ...
    file_text = (
        "📁 <b>FILE</b>\n"
        "______________________\n\n"
        "👇 Vui lòng chọn loại file bạn muốn mua:"
    )
...
def handle_callback_query(call):
    ...
    try:
        ...
        # 17b. Quay lại menu FILE (IMZ / FILZA) từ danh sách sản phẩm
        elif data == "file_menu_back":
            text = (
                "📁 <b>FILE</b>\n"
                "______________________\n\n"
                "👇 Vui lòng chọn loại file bạn muốn mua:"
